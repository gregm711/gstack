...
## Phase 1: System Understanding + Foundation Check (Cycle 0)
...
4. **Foundation check:** Is the code you're building on solid? Distinguish:
   - **Foundation problems:** code your change depends on that is broken, misleading, or structurally wrong. If you build on it, your work inherits the rot. Fix these first.
   - **Facade problems:** code *next to* your work that is ugly or outdated but doesn't affect your change. Leave these alone.
   - **The test:** "Will my change be wrong or fragile if I don't fix this?" Yes → foundation. No → facade.

If foundation issues exist, fix them as the first sub-goal (Cycle 1) before any feature work. Each fix gets its own commit with a message explaining the dependency: `fix: <what> — needed before <feature>`.
...
Foundation: solid | <issues found — will fix in Cycle 1>
Likely approach: ...
Estimated cycles: ...
```

Break the task into an ordered list of sub-goals. If foundation fixes are needed, they are sub-goal #1. Each cycle will tackle one sub-goal.
...
5. **Foundation re-check:** Did the previous cycle reveal new code you'll depend on? If so, is it solid? If you discover a foundation problem mid-loop, promote it to the next sub-goal. Don't build on code you just learned is broken. Don't go hunting — only flag what you encounter naturally while assessing.
...
- No facade cleanup — don't fix adjacent code that doesn't affect your change. (Foundation fixes get their own cycle, not mixed into feature cycles.)
...
Foundation work:
  - <what was fixed before building, or "none — ground was solid">
